    ...
    return (
        <div className="flex flex-col h-full bg-[#eaeff5]">
            ...
            <div className="px-5 pt-[calc(env(safe-area-inset-top,0px)+24px)] pb-3 bg-white/90 backdrop-blur-2xl sticky top-0 z-10 border-b border-slate-100">
                ...
                <div className="flex p-1 bg-slate-100/80 rounded-2xl mb-1 shadow-inner">
                    <button
                        onClick={() => { if (navigator.vibrate) navigator.vibrate(50); setActiveTab('matches'); }}
                        className={`flex-1 py-2.5 rounded-xl text-[13px] font-bold transition-all duration-200 active:scale-95 ${activeTab === 'matches' ? 'bg-white text-slate-900 shadow-[0_2px_8px_rgba(0,0,0,0.06)]' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Matches <span className={`ml-1 text-[10px] px-2 py-0.5 rounded-full ${activeTab === 'matches' ? 'bg-brand-primary/10 text-brand-primary' : 'bg-slate-200 text-slate-500'}`}>{matches.length}</span>
                    </button>
                    <button
                        onClick={() => { if (navigator.vibrate) navigator.vibrate(50); setActiveTab('sent'); }}
                        className={`flex-1 py-2.5 rounded-xl text-[13px] font-bold transition-all duration-200 active:scale-95 ${activeTab === 'sent' ? 'bg-white text-slate-900 shadow-[0_2px_8px_rgba(0,0,0,0.06)]' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Sent <span className={`ml-1 text-[10px] px-2 py-0.5 rounded-full ${activeTab === 'sent' ? 'bg-brand-primary/10 text-brand-primary' : 'bg-slate-200 text-slate-500'}`}>{sentLikes.length}</span>
                    </button>
                </div>
            </div>
